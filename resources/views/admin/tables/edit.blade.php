            ...
            <div class="flex flex-col m-4 p-4 bg-slate-100 px-6 py-6">
                ...
                <form method="POST" action="{{ route('admin.tables.update', $table->id) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 @error('guest_number') border-red-400 @enderror" aria-describedby="guest_number_help" type="number" min="1" max="20" step="1" id="guest_number" name="guest_number" value="{{ $table->guest_number }}">
                        @error('guest_number')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select id="status" name="status" class="form-multiselect w-full block @error('status') border-red-400 @enderror">

                            @foreach(\App\Helpers\Constant::TABLE_STATUS as $key => $value)
                                <option value="{{ $key }}" @selected($table->status == $key)>{{ $key }}</option>
                            @endforeach
                    ...
                    <div class="mb-6">
                        ...
                        <select name="location" id="location" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('location') border-red-400 @enderror" required>
                            ...
                            @foreach(\App\Helpers\Constant::TABLE_LOCATION as $key => $value)
                                <option value="{{ $key }}" @selected($table->location == $key)>{{ $key }}</option>
                            @endforeach
